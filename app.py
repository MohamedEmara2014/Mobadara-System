...
try:
    # قراءة البيانات مع ضمان عدم تأثر pandas بالدمج
    ...
    # التاريخ (الصف 2 المدمج) - نقرأه بحذر
    last_update_val = df_raw.iloc[0, col_idx_done - 1]
    if "تحديث:" in str(last_update_val):
        st.success(f"📅 {last_update_val}")

    # قراءة أسماء المشاريع (تبدأ من الصف الرابع - Index 2 في pandas)
    # الفلترة هنا تضمن أننا نأخذ الأسماء فقط ونتجاهل أي تشويش ناتج عن الدمج في الصفوف العليا
    ...
    if st.button(f"🚀 اعتماد وتصدير بيانات {selected_section}", type="primary", use_container_width=True):
        ...
        for i in range(len(edited_df)):
            ...
            # نرسل البيانات فقط إذا وجدت، ونبدأ من الصف 4
            if val_done or val_issues:
                # i=0 (أول مشروع) يذهب للصف 4 في الشيت
                target_row = i + 4
        ...
        if updates_to_send:
            with st.spinner("جاري المزامنة..."):
                response = requests.get(SCRIPT_URL, params={"updates": json.dumps(updates_to_send)})
                if response.status_code == 200:
                    st.success("✅ تم التحديث بنجاح! تم تجاوز مشكلة الدمج.")
                    st.balloons()
                    time.sleep(1)
                    st.rerun()
        else:
            st.warning("⚠️ يرجى إدخال بيانات قبل الاعتماد.")
