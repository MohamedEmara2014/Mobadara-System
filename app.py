...
try:
    ...
    # استخراج أسماء الأقسام (عمود للمشروع ثم عمودين لكل قسم)
    ...
    # عرض محرر البيانات بدون خاصية placeholder المسببة للخطأ
    ...
    if st.button(f"🚀 اعتماد وتصدير بيانات {selected_section} بالكامل", type="primary", use_container_width=True):
        ...
        for i in range(len(edited_df)):
            ...
            # i + 4 لأننا نبدأ من الصف الرابع في الشيت
        ...
        if updates_to_send:
            with st.spinner("جاري مزامنة البيانات..."):
                ...
                if response.status_code == 200:
                    st.success(f"✅ تم اعتماد تقرير {selected_section} بنجاح!")
                    st.balloons()
                    time.sleep(2)
                    st.rerun()
                else:
                    st.error("❌ فشل الاتصال. يرجى التأكد من إعدادات الـ Web App.")
        else:
            st.warning("⚠️ يرجى ملء بيانات الجدول أولاً.")

except Exception as e:
    st.error(f"تأكد من تنسيق الشيت. الخطأ الحالي: {e}")
